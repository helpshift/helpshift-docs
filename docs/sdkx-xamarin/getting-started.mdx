...
Initialize the SDK by calling `Install()` method before any other interaction with the Helpshift SDK. You can optionally pass the configuration dictionary to `Install()` API. To see supported configurations, check the [SDK Configuration](/sdkx_xamarin/sdk-configuration.mdx) page.
