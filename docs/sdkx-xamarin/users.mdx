...
{
    userData.Add("userEmail", userEmail);
}
...
If your use case involves multiple logged-in/anonymous users using the same device and discussing info during support conversations (which you ideally wouldn't want to be shared across logins), you should use the `Helpshift.ClearAnonymousUserOnLogin(bool clearAnonymousUser)` API.
...
```csharp
Helpshift.ClearAnonymousUserOnLogin(bool clearAnonymousUser);
```
...
{
    userData.Add("userAuthToken", userAuthToken);  
}
...
If identity verification fails, the SDK will invoke the `OnUserAuthenticationFailure(HelpshiftUserAuthFailureReason)` delegate method of `IHelpshiftEventsListener` interface to notify the application of the failure:
...
<table>
  ...
  <tr>
    <td>
      HelpshiftUserAuthFailureReason
      <br />
...
The authentication failure reason is wrapped in an enum `HelpshiftUserAuthFailureReason`.

Example:

```csharp
// ...
// Assuming this class implements IHelpshiftEventsListener interface...
...
{
    // Handle user authentication failure here.
}
```
