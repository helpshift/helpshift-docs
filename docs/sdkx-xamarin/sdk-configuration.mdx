...
Helpshift provides several configuration options that you can use to customize the behavior of the SDK. Use these options in conjunction with our [Helpshift APIs](/sdkx_xamarin/support-tools/).
...
namespace App
{
    ...
    public class MyApplication : Application
    {
        override public void OnCreate()
        {
            base.OnCreate();
            Dictionary<string, object> installConfig = new Dictionary<string, object>();
            installConfig.Add("screenOrientation", ActivityInfo.ScreenOrientation.Portrait);
            // Add any other install config parameters
            Helpshift.Install("YOUR_APP_ID", "YOUR_DOMAIN_NAME", installConfig);
        }
    }
...
Please check [design](/sdkx_xamarin/design) page to apply your styles and theming to the SDK.

## Tracking {#tracking}

This config represents the tracking of user actions. For more information, please check [tracking](/sdkx_xamarin/tracking).
