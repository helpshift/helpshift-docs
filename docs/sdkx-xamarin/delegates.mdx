...
If you have `User Authentication` feature enabled on the Dashboard and if you pass an invalid token in the `Helpshift.Login()`, then you will receive this event with reason string. Check [here for more info](/sdkx_xamarin/users).
