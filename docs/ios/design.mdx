...
If you want to change strings used in the SDK's UI or want to translate them as per device language, see [String Customization](/ios/string-customization).

<a id="skinning"></a>
## Theming and Skinning

<a id="default-skins"></a>
### Default Skinning Configurations

Helpshift comes with four default skinning configurations -

- **Light** - `HelpshiftCustomThemes/HelpshiftCustomConfig.plist`
- **Dark** - `HelpshiftCustomThemes/HelpshiftCustomConfigDark.plist`
- **Light High Contrast** - `HelpshiftCustomThemes/HelpshiftCustomConfigHighContrast.plist`
- **Dark High Contrast** - `HelpshiftCustomThemes/HelpshiftConfigCustomDarkHighContrast.plist`

<a id="custom-skinning"></a>
...
<a id="runtime-light-dark"></a>
#### If your app wants to change both light and dark themes at runtime, use the following API:
...
1. Go to the `HelpshiftCustomThemes` folder.
2. Make desired changes in the `HelpshiftCustomConfig.plist` file. For details, refer to the [Guide](#attributes-global)
3. Add this file to your project's bundle resources. Go to Build Phases → Copy Bundle Resources → Add the file using + icon.
4. Make desired change in the `HelpshiftCustomDarkConfig.plist` file and add it your project instructions mentioned in step 3
5. Call `setLightTheme:darkTheme:` API anytime after the SDK is initialized.

Example Code

```objc
[HelpshiftCore setLightTheme:@"HelpshiftCustomLightConfig" darkTheme:@"HelpshiftCustomDarkConfig"];
```

<Admonition type="info" title="Note">

1. Light and Dark Appearance switching works only if your app is built with Xcode 11 or above.
2. You can pass nil to reset the theme to default SDK theme for that param.
...
5. The [enableAutomaticThemeSwitching](/ios/sdk-configuration#enableAutomaticThemeSwitching) config flag is ignored when this API is used and SDK will use the provided themes for Light and Dark Appearances respectively.

</Admonition>

<a id="runtime-common"></a>
#### If your app does not support Dark Appearance and uses same theme irrespective of the OS Appearance, use the following API:

`setTheme`

Steps to follow:

1. Make desired changes in the custom plist file `HelpshiftCustomConfig.plist` inside the `HelpshiftCustomThemes` folder. For details, refer to the [Guide](#attributes-global)
2. Add this file to your project's bundle resources. Go to Build Phases → Copy Bundle Resources → Add the file using + icon.
3. Call `setTheme` API anytime after the SDK is initialized.

Example Code

```objc
[HelpshiftCore setTheme:@"HelpshiftCustomConfig"];
```

<Admonition type="info" title="Note">

1. You can pass nil to reset the theme to default SDK theme for that param.
...
3. The [enableAutomaticThemeSwitching](/ios/sdk-configuration#enableAutomaticThemeSwitching) config flag is ignored when this API is used and SDK will use the provided theme for both Light and Dark Appearances.

</Admonition>

<a id="compile-time-theming"></a>
### Compile time Theme Changes

If your app does not change theme on runtime, Please read the below documentation:

<a id="compile-time-light-dark"></a>
#### If your app supports Dark Appearance in iOS 13

Helpshift SDK bundles themes for light and dark mode out of the box but leaves the decision to enable dark mode switching to the application developers. You can enable automatic switching of Helpshift SDK theme using [enableAutomaticThemeSwitching](/ios/sdk-configuration#enableAutomaticThemeSwitching) flag.
...
If you are integrating the Helpshift SDK with Cocoapods, and want to skin your app, please follow the below steps:

- Open the `Pods/Helpshift/HelpshiftCustomThemes` folder and copy the `HelpshiftCustomConfig.plist` file into your Xcode app project.
- Change the plist values for theme elements that you want to customise. Helpshift SDK will use this file for Light Mode. For attribute details, refer to the [Guide](#attributes-global)
- Open the `Pods/Helpshift/HelpshiftCustomThemes` folder and copy the `HelpshiftCustomConfigDark.plist` file into your Xcode app project.
- Change the plist values for theme elements that you want to customise. Helpshift SDK will use this file for Dark Appearance. For attribute details, refer to the [Guide](#attributes-global)

**Integration from zip file**

If you want to customize the skinning attributes of the Helpshift SDK:

1. Change the values in the `HelpshiftCustomConfig.plist` and `HelpshiftConfigDark.plist` inside the `HelpshiftCustomThemes` folder .
1. Copy the `HelpshiftCustomConfig.plist` and `HelpshiftCustomConfigDark.plist` add both the files to your application's main bundle.
...
<a id="compile-time-common"></a>
#### If your app does not support Dark Appearance in iOS 13
...
1. Copy the `HelpshiftCustomConfig.plist` file and add this file to your application's main bundle.
...
<a id="attributes-global"></a>
### Global Attributes

To skin your app, make sure you set values for attributes in appropritate plist file.

<div className="text-sm ">

|        |                                                                                                                                                                                                                                                                                                                                                                                            |
| ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Colors | Colors are specified in hexadecimal format, e.g. <code>FF0000</code> is Red                                                                                                                                                                                                                                                                                                                |
| Images | Images should be included in your project and can be specified by their filename. Helpshift supports iOS image naming convention (i.e. @2x images, if found are utilized on retina displays)                                                                                                                                                                                               |
| Fonts  | Font names should be specified by their family name & style. For example, **Futura-Medium** refers to Futura _family_ of medium _style_. <br /> Checkout [iosfonts.com](http://iosfonts.com) for fonts available on SDK supported iOS versions. You can also use fonts other than these if they're used by your project. See [using custom fonts](#custom-fonts) section for more details. |
...
|                                        |                                                                                                                                                      |
| -------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| Font name                              | Global font face - applies to texts for sections list, FAQ list, FAQ answer, search results and notification banner subtitle.                        |
| Bold font name                         | Applies to FAQ section headers on support screen, FAQ question and notification banner title                                                         |
...
<a id="attributes-navigation-bar"></a>
### Navigation Bar
...
| Field Background color             | Color for background of the reply field.                                                        |
| Field border color                 | Color for the reply field border.                                                               |
...
<a id="attributes-message-retry-badge"></a>
### Message Retry Badge
...
<a id="conversation-resolution"></a>
