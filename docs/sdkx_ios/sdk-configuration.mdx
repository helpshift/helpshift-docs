...
In config, setting the `initiateChatOnLoad` option to `YES` will start a new conversation, if the previous issue is resolved, without the end user clicking the New Conversation button or going through the post resolution flows like feedback bots of the previous issue.
