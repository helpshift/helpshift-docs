...
## initiateChatOnLoad {#initiateChatOnLoad}

Upon setting `initiateChatOnLoad` to `YES`, when the conversation loads if the previous issue is resolved, a new conversation will be started, without end user clicking on the `New Conversation` button or going through the post resolution flows like feedback bots of previous issue.
