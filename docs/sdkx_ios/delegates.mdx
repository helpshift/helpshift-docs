...
- Event Data: 
    - `HelpshiftEventDataMessageType` <br />
    - `HelpshiftEventDataMessageTypeText` <br />
    - `HelpshiftEventDataMessageTypeAttachment` <br />
    - `HelpshiftEventDataMessageBody` <br />
...
If you want a count of all new messages received in an existing conversation, you can call this API `[Helpshift requestUnreadMesssageCount:];`.
...
If you call the method `[Helpshift requestUnreadMessageCount:YES];` it will return a notification count from server in the above delegate method. The notification count here can be fetched either from the cache or from the Helpshift servers, the count from Helpshift’s servers is rate limited and it returns the value only if a subsequent call is made to the API, after the reset timeout or when the user just closes the chat screen (whichever is earlier). For an active issue, the reset timeout is 1 minute and 5 minutes for inactive issues.
