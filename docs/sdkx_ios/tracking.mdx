...
This API is meant to be used along with the new [User Identity system](/sdkx_ios/identity-user-hub). Use this API to update global attributes associated with the logged in user.
...
If the attributes dictionary contains an unsupported key, an error event will be communicated via the delegate.
If the attributes dictionary contains supported key, but with unsupported value type, the entry will be ignored.
