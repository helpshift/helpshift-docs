...
<td>No</td>
<td>
   Used to specify the functionality for segmenting FAQs based on tags,
   ensuring the effective delivery of relevant information (FAQs) to end users.
   A JSON object must contain two mandatory nested keys - `tags`  and `operator`. <br />
   The `operator` value can be any of the following: `and`, `or`, `not`;
   otherwise, the API will respond with an 400 error response. <br />
   The tags key is a JSON array of strings, and at least one tag must be specified; otherwise, the API will
   send an 400 error response. <br />
   Example value for `help_faq_filter` -

   ```json
   {"tags": ["tag1", "tag2"], "operator": "and"}
   ```
   In this example, the API will send a 201 response containing a URL link.
   The page linked to this URL will exclusively display FAQs associated with both `tag1` and `tag2`.
   If no FAQs meet the specified criteria, none will be shown on the page. <br />
