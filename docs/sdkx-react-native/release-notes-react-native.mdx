...
SDKX React Native 10.3.0 now supports React Native v0.70.15 & above.
...
- `Helpshift.login()` API now returns a boolean indicating whether login was successful or not. This change is in existing API to introduce mandatory validations for login parameters and hence this might affect existing code compilation. For more details, [refer here](/sdkx-react-native/users/#logging-in-users).
- Added support for attachments in Smart FAQs.
