...
- Trigger a [Proactive Engagement](/sdkx-react-native/proactive-engagement) notification for the targeted user.

<ConfigurePushNotificationsiOS />

<a id="configure-helpshift-react-native-sdk"></a>
...
Alternatively, you can choose to integrate notifications natively. For detailed steps on native integration, please refer to the [Native iOS Notifications](/sdkx_ios/notifications/) guide.

If you choose the native integration approach, please ensure that the Helpshift [`install`](/sdkx_ios/getting-started/#initializing/) call is also made from the native code.
...
For foreground notifications (specifically the `list` option) to work correctly on iOS 14+, you need to apply a patch to the `react-native-notifications` package. Please refer to the [Troubleshooting](/sdkx-react-native/troubleshooting#foreground-notifications-ios) page for details.
