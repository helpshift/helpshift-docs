...
You need to generate certificates for APNS on Apple's portal that you can later provide to Helpshift. This lets Helpshift send push notifications to your users. Apple has moved away from the old type of certificates to Apple Push Notification service SSL. There is another option to create Apple Push Notification Authentication Key, but Helpshift does not support this option yet. It's also possible that you still have the old type of certificate that has not yet expired, this is also supported by Helpshift.
...
Here we are providing a sample example for push notification use. [react-native-notifications](https://www.npmjs.com/package/react-native-notifications) package. You can use any package your organization uses.
...
```
  Notifications.events().registerNotificationOpened(
    (notification: Notification, completion: () => void) => {
      ...
      const notificationData = notification.payload;
      //This is for manage outbond support link.
      const doesNotificationDataHaveProactiveLink =
        notificationData.hasOwnProperty('helpshift_proactive_link');
      const doesNotificationDataHaveOrigin =
        notificationData.hasOwnProperty('origin') &&
      ...
      if (doesNotificationDataHaveProactiveLink) {
        handleProactiveLink(notificationData.helpshift_proactive_link, <config_object>);
      } else if (doesNotificationDataHaveOrigin) {
...
The Helpshift [Init](/sdkx-react-native/getting-started/) call supports flags for configuring SDK behavior.
