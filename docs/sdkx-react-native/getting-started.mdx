...
Call the `install` function before any other interaction with the Helpshift SDK in `App.tsx`.(It should be used in the useEffect() hook callback in react native call)
