...
<ul>
  ...
  <li>
    When the user ID matches an existing user, that user's email will be
...
If your use case involves multiple logged-in/anonymous users using the same device and discussing info during support conversations (which you ideally wouldn't want to be shared across logins), you should use the `clearAnonymousUser(boolean clearAnonymousUser)` API.
...
<table>
  ...
  <tr>
    ...
    <td>
      ...
      When the 'user authentication token' is invalid, end-users will not be able
      to file Issues and an error is shown to them, as mentioned previously. You
      can use this delegate if you want to show your alerts or re-fetch the
...
The authentication failure reason is wrapped in an enum `EHelpshiftAuthenticationFailureReason`
