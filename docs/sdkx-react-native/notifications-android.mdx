...
For example with [React Native plugin](https://rnfirebase.io/messaging/usage), you can call the `const token = await requestNotificationPermission()` function to fetch the push registration token. Register this token with Helpshift SDK via the `registerPushToken(<token>)` function.
