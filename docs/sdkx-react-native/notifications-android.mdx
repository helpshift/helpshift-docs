...
You can use any Firebase plugin from React Native Plugin. For ex - [React Native Firebase](https://rnfirebase.io/messaging/usage)
...
First of all, you will need to setup firebase push notification plugin where you will be getting firebase token
...
For example with [React Native plugin](https://rnfirebase.io/messaging/usage), you can call `const token = await requestNotificationPermission()` function to fetch the push registration token. Register this token with Helpshift SDK via `Register Push Token` function.
...
To handle notification from background we recommend to call `setBackgroundMessageHandler` inside index.js file where your AppRegistry.registerComponent is called.
```
...
helpshiftEmitter.addListener('onEventOccurred', (data) => {
      if (data.eventName == 'receivedUnreadMessageCount') {
        try {
          ...
          console.log(countData.count); // show toast to user for count
        } catch (err) {
