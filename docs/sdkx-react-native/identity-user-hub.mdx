...
loginWithIdentity(jwtToken, {"full_privacy_enabled": false})
      .then(() =>  {
            // Login is successful
            console.log("Helpshift - IdentityLogin - Login was successful!")}
        ).catch((err) => {
            console.log("Helpshift - IdentityLogin - Login was Failure!", err)}
        );
```
...
For more details on these events, please check the [Delegates](/sdkx-react-native/delegates/#user_session_expired) page.
...
```javascript
import {addUserIdentities} from "helpshift-plugin-sdkx-react-native";
// other imports
...
This API leverages the existing Helpshift delegates system to notify the app in case of any errors. For more details on the error events that can be triggered by this API, please refer the [Delegates](/sdkx-react-native/delegates/#user-identity-system-events) page.

### Troubleshooting and tips

Login failure can be detected by listening to the [HelpshiftUserLoginEventsListener](/sdkx-react-native/identity-user-hub/#logging-in-your-end-users) listener. The possible reasons for login failure are cited in the [Login failure reasons](/sdkx-react-native/identity-user-hub/#login-failure-reasons) section.
...
let masterAttributes = {"<master-attribute-key>", "<master-attribute-value>"}

updateMasterAttributes(masterAttributes);
...
let appAttributes = {"<app-attribute-key>", "<app-attribute-value>"}

updateAppAttributes(appAttributes);
...
The constants for these failure reasons are defined in `helpshiftConstants.ts` file.
