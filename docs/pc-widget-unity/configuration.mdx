...
Determines how the widget resizes. Example values include `ResizeMode.SlimRight`, `ResizeMode.SlimCenter`, `ResizeMode.SlimLeft`, `ResizeMode.WideCenter`, and `ResizeMode.FullScreen`.

<Admonition type="info" title="Note">
By default, the widget is set to <code>ResizeMode.SlimRight</code>.
</Admonition>
