...
- Salesforce integration will support LanguageAI, which allows agents to converse with multi-lingual end users.
- Salesforce messenger will support 47 languages out of the box, with English as default. Details of the supported languages can be found [here](https://developers.helpshift.com/android/i18n/).
- Agents can now reply to customers in their languages and translate customer replies to English.
- Bug fixes

<Admonition type="info" title="Note">
3.10 package install/upgrade

**Action needed:**

We are facing some issues in updating the listing on Salesforce appExchange.
Please contact Helpshift support for install/upgrade of 3.10 package.
...
- Text direction in messages will be detected automatically, and agents can set the text direction manually.
- Agents will now be able to assign issues (Cases) to bots enabling them to leverage the power of Helpshift bots in Salesforce messenger.
- Smart intents will now be visible in Salesforce messenger.
- Bug fixes
...
- Case comments for closed cases can now be archived. Admins will see an option to archive data in : Helpshift Admin (LWC) -> Settings -> Archive data.
  - This will help in reducing storage for Helpshift Cases in Salesforce.
- Added fixes for improving package security.
- Bug fixes
