...
1. Things to check after upgrade \*\*\* IMP
   1. Verify whether the following picklist values are present in **Helpshift_Message_Type__c** field. This field can be found on the Helpshift Case Comment object from the object manager. [Add the new ones]
   ```text
   bot_control_message
   text
   faq
   ...
   <Image
     ...
     width="full"
   />
   2. Check if Helpshift_Standard_Object permission set has the new Apex classes permissions. If
   not, add them directly as shown below.
