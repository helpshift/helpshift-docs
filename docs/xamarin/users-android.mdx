...
- If you are setting [`EnableFullPrivacy`](/xamarin/sdk-configuration-android#enableFullPrivacy) to be True, you shouldn't be using email as the only identifier in the LoginAPI for that user. This will result in the creation of an anonymous user.
...
- If you are using the [`leaveBreadCrumb`](/xamarin/tracking-android#app-usage) API to pass user login related crumbs, please make sure to call the clearBreadCrumbs API when you change the login.
...
When Identity Verification is turned 'On' and a login request is made with no 'user authentication token' or with an invalid 'user authentication token' for Xamarin SDK v3.0.0 & above, identity verification will fail. If identity verification fails, the following will hold true:
...
End-users on Xamarin SDK 2.x or lower are not impacted, even if 'Identity Verification' is enabled on the Dashboard.
...
<table>
  ...
  <tr>
    ...
    <td>If you do not plan on sending a 'user authentication token' when implementing Xamarin SDK 3.0.0, but plan on implementing it in the future, you can use this failure delegate to show your own alerts to the user, such as a prompt to update the app. You may want to use this if you are using the login API without the 'user authentication token', since these users will be considered unverified once 'Identity Verification' is enabled on the Dashboard. Using this delegate is completely optional - Helpshift will prevent unverified users from being able to file Issues, as mentioned previously.</td>
  </tr>
  <tr>
    <td>INVALID_AUTH_TOKEN</td>
    <td>When the 'user authentication token' is invalid</td>
    <td>
	For Xamarin SDK v3.0.0 and later, if the HMAC Digest being provided via login API is invalid, then Helpshift will prevent the users from filing Issues. The 'user authentication token' can be invalid if:
...
<a id="identity-verification-troubleshooting"></a>
