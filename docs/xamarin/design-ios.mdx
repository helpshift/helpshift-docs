...
If you want to change strings used in the SDK's UI or want to translate them as per device language, see [String Customization](/xamarin/string-customization-ios).
...
`SetThemes(string lightThemeFileName, string darkThemeFileName)`
...
1. Make a copy of `HelpshiftConfig.plist` and rename it to your preferred name, for eg `HelpshiftCustomLightConfig.plist`
2. Make desired changes in your custom file `HelpshiftCustomLightConfig.plist` file. For details, refer to the [Guide](#attributes-global)
3. Add this file to your project's Resources folder.
4. Similarly make a copy of `HelpshiftConfigDark.plist` and rename it your preferred name, for eg `HelpshiftCustomDarkConfig.plist`
5. Make desired change in your custom file `HelpshiftCustomDarkConfig.plist` and add it your project instructions mentioned in step 3
6. Call `SetThemes(string lightThemeFileName, string darkThemeFileName)` API anytime after the SDK is initialized

Example Code

```csharp
HelpshiftCore.SetThemes("HelpshiftCustomLightConfig", "HelpshiftCustomDarkConfig");
```
...
5. The [enableAutomaticThemeSwitching](/xamarin/sdk-configuration-ios#enableAutomaticThemeSwitching) config flag is ignored when this API is used and SDK will use the provided themes for Light and Dark Appearances respectively.
...
`SetTheme (string themeFileName)`

Steps to follow:

1. Make a copy of `HelpshiftConfig.plist` and rename it to your preferred name, for eg `HelpshiftCustomConfig.plist`
2. Make desired changes in your custom file `HelpshiftCustomConfig.plist` file. For details, refer to the [Guide](#attributes-global)
3. Add this file to your project's Resources folder.
4. Call HelpshiftCore.SetTheme("HelpshiftCustomConfig"); API anytime after the SDK is initialized
...
3. The [enableAutomaticThemeSwitching](/xamarin/sdk-configuration-ios#enableAutomaticThemeSwitching) config flag is ignored when this API is used and SDK will use the provided theme for both Light and Dark Appearances.
...
Helpshift SDK bundles themes for light and dark mode out of the box but leaves the decision to enable dark mode switching to the application developers. You can enable automatic switching of Helpshift SDK theme using [enableAutomaticThemeSwitching](/xamarin/sdk-configuration-ios#enableAutomaticThemeSwitching) flag.
...
1. Change the values in the `HelpshiftConfig.plist` and `HelpshiftConfigDark.plist`.
1. Copy the `HelpshiftConfig.plist` file as `HelpshiftCustomConfig.plist` and copy the `HelpshiftConfigDark.plist` file as `HelpshiftCustomConfigDark.plist`. Add both the files to your application's main bundle.
...
1. Change the values in the `HelpshiftConfig.plist` and `HelpshiftConfigDark.plist`.
1. Copy the `HelpshiftConfig.plist` file as `HelpshiftCustomConfig.plist` and add this file to your application's main bundle.
...
If you are having issues with skinning the SDK, head over to the [Troubleshooting](/xamarin/troubleshooting-ios#skinning) section for further information.
