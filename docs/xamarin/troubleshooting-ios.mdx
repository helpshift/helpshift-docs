...
<table>
    ...
    <tr>
        <td>1</td>
        <td>Log collection api does not send logs along with issues when the assemble name and application names are different</td>
        <td>v2.0.0</td>
        <td>N/A</td>
        <td>If the assembly name and the application do not match exactly then log collection does not report logs along with the issue on admin dashboard. This is a limitation on log collection queries on iOS devices.</td>
        <td>Keep the assembly name of the application same as the application name.</td>
    </tr>
</table>
