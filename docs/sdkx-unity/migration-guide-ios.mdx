...
<Steps title="Migrate from legacy SDK Unity plugin to SDK X Unity plugin">
    <Step number="1" title="Check migration support" subtitle="Go through migration guide" link="/sdkx-unity/migration-guide-ios#unsupported-features" newTab={true} />
    <Step number="2" title="Update to SDK X Unity plugin" subtitle="Migrate code/features step by step" link="/sdkx-unity/migration-guide-ios#update-helpshift-dependencies" newTab={true} />
...
* [Conversation pre-fill text](/unity/sdk-configuration-ios#conversationPrefillText)
