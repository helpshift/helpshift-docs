...
#### Steps to support push notification in background or killed state

If you are unable to delegate the push notification payload from the Java receiver class to C# api's then, for such cases, you can use the native Java api `HelpshiftUnity.handlePush(Context context, Map<String, String> data)` to handle push notifications from Helpshift.

You can use below sample project for integration of push notification in java layer. This project routes Helpshift push notifications directly to the native Helpshift SDK for reliable background and killed-state handling, while forwarding all non-Helpshift notifications to Unity’s default FCM service to avoid conflicts with other SDKs.


  1. In your Unity project, navigate to `Assets/Plugins/Android`
  2. From the unzipped Helpshift Unity SDK, copy [`HelpshiftBackgroundFCM.androidlib`](/sdkx-unity/getting-started-android#getting-sdk) folder to above path.

<Admonition type="info" title="Note">

* If your application is integrating multiple SDKs then you need to handle their push notifications handling in this Java layer as well.
* `HelpshiftBackgroundFCM.androidlib` is not available in package published via Unity Asset Store. For getting this file, please download the package from [download section](/sdkx-unity/getting-started-android/#getting-sdk) and unzip the downloaded file.
</Admonition>

`HelpshiftCustomFCMService` is a custom Firebase Cloud Messaging (FCM) service implemented at the native Android (Java) layer to ensure reliable handling of Helpshift push notifications, especially when the app is in the background or killed state, where Unity and C# APIs may not be initialized.

1. The service receives all incoming FCM messages in `onMessageReceived`.
2. It checks for origin field in notification payload.
3. If origin is "helpshift", the notification is handled directly by the native Helpshift SDK using `HelpshiftUnity.handlePush(context, data)`. This guarantees correct processing even when Unity is not running.
4. When multiple SDKs are integrated, their push notifications should be handled in this Java layer by routing messages to the appropriate SDK based on the payload.
5. If the notification is from a non-Helpshift source, it forwards the message to Unity’s default FCM handling by calling `super.onMessageReceived(message)`.

In the Java implementation of the push notification receiver, you can use the following API as shown in the example below.
