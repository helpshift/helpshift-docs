...
- Locate the `HelpshiftX.framework.dSYM` file in package downloded from the [Getting Started](/sdkx-unity/getting-started-ios/#getting-sdk) page.
...
### Unity 6000.x Specific Issues

#### Cyclic Dependency Error During iOS Build

When building an iOS app using **Unity 6000.x**, you may encounter a cyclic dependency or build phase cycle error related to the `Unity-iPhone` target.

**Resolution**:
Pull **Unity Process Symbols** for the `Unity-iPhone` target in Xcode to the bottom.

Steps:
1. Open the generated Xcode project.
2. Select the **Unity-iPhone** target.
3. Navigate to **Build Settings**.
4. Pull **Unity Process Symbols for Unity-iPhone** to the bottom

This resolves known cyclic dependency issues observed in Unity 6000.x iOS builds.
