...
1. Use `standalone jetifier` tool to manually jetify the Helpshift library file:
   `<project>/Assets/Plugins/Android/Helpshift.aar`
1. Replace the existing `Helpshift.aar` files with the **file generated from the standalone jetifier tool**.
