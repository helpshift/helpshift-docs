...
You need to obtain a JWT for new identities from your dedicated endpoint. Please check the [Prerequisites section](/sdkx-unity/identity-user-hub-android/#prerequisites) for more details.

This API leverages the existing Helpshift delegates system to notify the app in case of any errors. For more details on the error events that can be triggered by this API, please refer the [Delegates](/sdkx-unity/delegates-android.mdx/#user-identity-system-events) page.

### Troubleshooting and tips

Login failure can be detected by listening to the [HelpshiftUserLoginEventsListener](/sdkx-unity/identity-user-hub-android/#logging-in-your-end-users) listener. The possible reasons for login failure are cited in the [Login failure reasons](/sdkx-unity/identity-user-hub-android/#login-failure-reasons) section.
