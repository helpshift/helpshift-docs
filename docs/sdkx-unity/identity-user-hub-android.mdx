...
- When setting `full_privacy_enabled` to true, it is encouraged to use `uid` as identifier and other identifiers will be dropped as potential PLL data.
...
This API should be called only once for a unique set of identities. Helpshift will keep adding identity even if same set of identities is passed in again.It won't de-duplicate the identity before adding.
