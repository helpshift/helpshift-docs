...
- Added support for User Hub (User Identity & Profiles) and related user data collection APIs.
  - For more details on user identities, refer [User Identities](/sdkx_android/identity-user-hub/) page ( Android ) and [User Identities](/sdkx_android/identity-user-hub/) page ( iOS ) .
  - For more details on user data collection, refer [Tracking](/sdkx_android/tracking/#update-global-user-data) page ( Android ) and [Tracking](/sdkx_android/tracking/#update-global-user-data) page ( iOS ).
...
- Included debug symbols (dsym) file inside the framework.
  - The disk size has been increased due to this addition.
  - Does not affect the size of the HelpshiftX binary.
  - dsym files are optional and can be removed if not needed from these locations: `HelpshiftX.xcframework/ios-arm64_armv7/dSYMs/` and `HelpshiftX.xcframework/ios-arm64_i386_x86_64-simulator/dSYMs/`.
