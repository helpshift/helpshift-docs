...
- All the public APIs in the SDK should be called after initializing the SDK via [Helpshift.install() API](/sdkx-unity/getting-started-android#initializing)
...
|               |                                        |
| ------------- | -------------------------------------- |
| API:       | <code>Helpshift.PauseDisplayOfInAppNotification()</code>   |
...
```csharp
  ...
  void PauseDisplayOfInAppNotification()
  {
      //... Assuming Helpshift.install() is already called.

      // based on state of your app, decide to pause/unpause proactive engagement notifications.
      help.PauseDisplayOfInAppNotification(true/false);
  }
```
...
- Note: You need to call this API after `Helpshift.install() API and before Helpshift.HandlePushNotification()`
