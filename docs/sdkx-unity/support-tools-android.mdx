...
```java
  ...
  HelpshiftSdk.SdkVersion();
```

## Clossing a Helpshift Session {#close-session}

You can use `HelpshiftSdk.CloseSession` API to close an active SDK session. If no SDK session is active, calling this API will have no effect.
