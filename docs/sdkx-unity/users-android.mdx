...
This section describes how you can integrate users with the legacy method. For integrating with the new Identity System `User Hub`, please refer [this page](/sdkx-unity/identity-user-hub-android).
...
You need to be aware of some considerations when using the `HelpshiftSdk.Login()` API -
...
From 10.3.0, `HelpshiftSdk.Login()` API will return a boolean indicating whether login was successful or not. This change is in existing API to introduce mandatory validations for login parameters and hence this might affect code compilation.
...
If your use case involves multiple logged-in/anonymous users using the same device and discussing info during support conversations (which you ideally wouldn't want to be shared across logins), you should use the `_support.ClearAnonymousUserOnLogin(shouldClear)` functionality.
