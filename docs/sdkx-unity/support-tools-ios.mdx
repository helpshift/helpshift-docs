...
# Helpshift APIs <small>iOS</small>
...
You can use the API `CloseSession()` to close an active SDK session. Calling this API will close any open SDK screens and invoke the corresponding [lifecycle delegate methods](/sdkx-unity/delegates-ios/#session-delegates). If no SDK session is active, calling this API will have no effect.
...
For more info about about this API and languages you can refer [here](/sdkx-unity/i18n-ios/)

## User Hub APIs
Our new Identity system - [User Hub](/sdkx-unity/identity-user-hub-ios/), introduces various APIs for managing users and their data. Refer to the linked section for details.