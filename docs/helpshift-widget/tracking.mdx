...
- This config should be called before the Issue is created (i.e. before the Webchat launcher is clicked or the "[open](/web-chat/api#open-widget)" API is called). You won't be able to update Custom Issue Fields after the Issue is created.
- If any of the keys, types or values are invalid, that particular Custom Issue Field will get dropped.
- If you want to update the Custom Issue Fields for subsequent issue creations, you need to update the cifs config in the helpshiftWidgetConfig object and then use the [updateHelpshiftWidgetConfig](https://developers.helpshift.com/helpshift-widget/api/#update-helpshift-widget-config) API to update the Custom Issue Fields for subsequent issue creations.
