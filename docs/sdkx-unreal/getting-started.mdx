...
## Initializing Helpshift in your app (Blueprint) {#initializing}
...
The `Init` call takes a map of the additional parameters. All of the parameters are optional, which means you can pass an empty map. The meaning of different keys and their possible values can be found [here](/sdkx-unreal/sdk-configuration)

## Initializing Helpshift in your app (Code) {#initializing-code}
...
For iOS, if you want to support push notifications as described in the [Notifications page](/sdkx-unreal/notifications-ios), you **must** call `InitWithCredentials` from `application:didFinishLaunchingWithOptions` method in your AppDelegate.
...
Refer [User Hub documentation](/sdkx-unreal/identity-user-hub/#user-hub-implementation-flow-diagram) for SDK integration details.