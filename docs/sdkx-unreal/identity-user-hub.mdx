...
import AppLevelUserAttributesList from "@site/docs/_partials/_userHubAppLevelAttributesList.mdx";
...
![](/static/books/unreal/login_with_identity.png)
...
```
static void LoginWithIdentity(const FString& IdentityJWT,
                              const TMap<FString, FHelpshiftConfigParameter>& LoginConfig,
                              const FHelpshiftLoginSuccessDelegate& SuccessCallback,
                              const FHelpshiftLoginFailureDelegate& FailureCallback);
```
...
For more details on these events, please check the [Delegates](/sdkx-unreal/delegates/#user-session-expired) page.
...
![](/static/books/unreal/add_user_identities.png)

</Centered>

You need to obtain a JWT for new identities from your dedicated endpoint. Please check the [Prerequisites](/sdkx-unreal/identity-user-hub/#prerequisites) section on this page for more details.

This API leverages the existing Helpshift delegates system to notify the app in case of any errors. For more details on the error events that can be triggered by this API, please refer the [Delegates](/sdkx-unreal/delegates/#user-identity-system-events) page.

### Troubleshooting and tips

Login failure can be detected by listening to the [FailureCallback](/sdkx-unreal/identity-user-hub/#logging-in-your-end-users). The possible reasons for login failure are cited in the [Login failure reasons](/sdkx-unreal/identity-user-hub/#login-failure-reasons) section.
...
![](/static/books/unreal/update_master_attribute.png)
![](/static/books/unreal/add_master_attribute.png)
![](/static/books/unreal/add_master_tag_attribute.png)
![](/static/books/unreal/add_master_cuf_attribute.png)
...
![](/static/books/unreal/update_app_attribute.png)
![](/static/books/unreal/add_app_attribute.png)
![](/static/books/unreal/add_app_tag_attribute.png)
![](/static/books/unreal/add_app_cuf_attribute.png)

</Centered>

App specific attributes must be one of the following types -

<AppLevelUserAttributesList />
...
## Login failure reasons {#login-failure-reasons}
...
The constants for these failure reasons are defined in `HelpshiftConstants.h` file.
