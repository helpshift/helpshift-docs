...
You can use the API `getSDKVersion` to get the current version of Helpshift SDK.

<Admonition type="info" title="Note">

This API is available only from 10.3.0 versions
...
You can use the API `Helpshift.closeSession` to close an active SDK session. If no SDK session is active, calling this API will have no effect.
