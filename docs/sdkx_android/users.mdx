...
```java
    ...
    userData.put("userId", "12234454");
    userData.put("userEmail", "captain@example.com");
    userData.put("userName", "Captain Planet");
...
<table>
  ...
  <tr>
    <td>HelpshiftAuthenticationFailureReason<br />.REASON_AUTH_TOKEN_NOT_PROVIDED</td>
  ...
  <tr>
    <td>HelpshiftAuthenticationFailureReason<br />.REASON_INVALID_AUTH_TOKEN</td>
    <td>When the 'user authentication token' is invalid</td>
    <td>
      If the HMAC Digest being provided via LoginAPI is invalid, then Helpshift will prevent the users from filing Issues. The 'user authentication token' can be invalid if:
      <ul>
