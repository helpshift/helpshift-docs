...
If you are on/moving to SDK X, you need to ensure that you are supporting installing your apps/games only on API Level 24 and above [Android OS 7 and above]. After SDK X v10.3.0 is released, you can start supporting API Level 21 and above.
...
Initialize SDK by calling `com.helpshift.Helpshift.install()` method.
 ...
 You can optionally pass to configuration map to `Helpshift.install()` API. To see supported configurations, check the [SDK Configuration](/sdkx_android/sdk-configuration.mdx) page.
