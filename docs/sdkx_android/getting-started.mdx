...
* SDK X is functional only above API Level 21 i.e Android OS 5 Lollipop and above.
...
* If your app is installed on a device below API level 21 (i.e below Android OS 5 Lollipop) then you can choose to have a different medium for providing support. For example, Email, Web etc. We throw an `UnsupportedOSVersionException` if the SDK is initialized below API Level 21. Please catch and use this exception according to your needs. Refer: [Initializing the SDK](/sdkx_android/getting-started/#initializing).

<Admonition type="info" title="Note">

  Update (August 2023): SDK X 10.2.x and above is functional only above API Level 24 i.e Android OS 7 Nougat and above
...
<TabItem value="<= 10.2.X>" label="10.2.X">
