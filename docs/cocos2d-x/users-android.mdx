...
- If you are setting [`enableFullPrivacy`](/cocos2d-x/sdk-configuration-android#enableFullPrivacy) to be "yes", you shouldn't be using email as the only identifier in the login API for that user. This will result in the creation of an anonymous user.
...
- If you are using the [`leaveBreadCrumb`]/cocos2d-x/tracking-android#app-usage) API to pass user login related crumbs, please make sure to call the [`clearBreadCrumbs`](/cocos2d-x/tracking-android#app-usage) API when you change the login.
