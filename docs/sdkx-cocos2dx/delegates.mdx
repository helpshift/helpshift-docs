...
You can find list of all Helpshift Events in `h` file.

To use Helpshift Events, include `h` in your UI file.
...
- Event data:
  ...
  | DATA_ATTACHMENTS | Map<String, Object>> | Attachments, if they are present |
...
void handleHelpshiftEvent(const char *eventName, cocos2d::ValueMap &eventData) {
    if (strcmp(eventName, AGENT_MESSAGE_RECEIVED) == 0) {
        ...
        std::string attachments = eventData[DATA_ATTACHMENTS].asString();
        std::string url = eventData[DATA_URL].asString();
        std::string contentType = eventData[DATA_CONTENT_TYPE].asString();
        std::string fileName = eventData[DATA_FILE_NAME].asString();
        std::string size = eventData[DATA_SIZE].asString();    
