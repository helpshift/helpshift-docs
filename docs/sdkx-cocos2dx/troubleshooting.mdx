---
sidebar_position: 150
title: Troubleshooting
description: Learn more about common SDK problems and their solutions.
---

import UserClickOnActionDelegateIssue from "@site/docs/_partials/_troubleShootingUserClickOnActionDelegate.mdx";
import ProguardRuleChatHelpcenter from "@site/docs/_partials/_androidProguardRules_ChatHelpcenter.mdx";
import ProguardRuleContentProvider from "@site/docs/_partials/_androidProguardRules_ContentProvider.mdx";

# Troubleshooting {#troubleshoot}

## Android SDK Integration

### Proguard rules

<ProguardRuleChatHelpcenter /> 

<ProguardRuleContentProvider /> 

---
## Known Issues {#known-issues}

### Orientation changes might lead to dismissal of the currently shown in-app notification

With v10.5.0, Proactive Engagement In-App notifications may get dismissed if the device orientation changes while the notification is visible. This occurs due to inconsistent system callbacks during orientation change in Cocos-based applications.

<UserClickOnActionDelegateIssue />
