...
This section describes how you can integrate users with the legacy method. For integrating with the new Identity System `User Hub`, please refer [this page](/sdkx-cocos2dx/identity-user-hub).
...
<a id="identify-and-manage-users"></a>
## How to Identify and Manage Users

<a id="logging-in-users"></a>
### Logging in users
...
You need to be aware of some considerations when using the `HelpshiftCocos2dx::login()` API -
...
- From 10.3.0, HelpshiftCocos2dx::login() API will return a boolean indicating whether login was successful or not. This change in existing API is to introduce mandatory validations for login parameters and hence this might affect code compilation.
...
std::string userEmail = fetchUserEmail();
std::string userName = fetchUserName();

if (userId.empty() && userEmail.empty())
{
    cocos2d::log("userId and userEmail both are empty. Invalid login!");
    return;
}

if (!userId.empty())
{
    userData["userId"] = cocos2d::Value(userId);
}

if (!userEmail.empty())
{
    userData["userEmail"] = cocos2d::Value(userEmail);
}

if (!userName.empty())
{
    userData["userName"] = cocos2d::Value(userName);
}

std::string userAuthToken = generateUserAuthToken();
if (!userAuthToken.empty())
{
    userData["userAuthToken"] = cocos2d::Value(userAuthToken);
}

bool isLoginSuccess = HelpshiftCocos2dx::login(userData);
```

<a id="logging-out-users"></a>
### Logging out users
...
If your use case involves multiple logged-in/anonymous users using the same device and discussing info during support conversations (which you ideally wouldn't want to be shared across logins), you should use the `HelpshiftCocos2dx::clearAnonymousUserOnLogin(bool clearAnonymousUser)` API.
...
```cpp
HelpshiftCocos2dx::clearAnonymousUserOnLogin(bool clearAnonymousUser);
```

<a id="identify-users"></a>
...
HelpshiftCocos2dx::login(userData);
```

<a id="identity-verification-failure"></a>
...
The authentication failure reason is wrapped in an enum `HSAuthenticationFailureReason`.

Example:

```cpp
void handleHelpshiftEvent(const char *eventName, cocos2d::ValueMap &eventData) {
    // handle Helpshift events
    ...
}
...
<a id="identity-verification-testing-troubleshooting"></a>
