...
If you want to change strings used in the SDK's UI or want to translate them as per device language, see [String Customization](/unity/string-customization-ios/.
...
`HelpshiftSdk.setThemes(lightThemeFileName, darkThemeFileName)`
...
1. Navigate to the `Assets/Helpshift/Plugins/iOS/HelpshiftCustomThemes` folder.
...
4. Similarly make a copy of `HelpshiftCustomConfigDark.plist` and rename it your preferred name, for eg `HelpshiftCustomDarkConfig.plist`
5. Make desired changes in your custom file `HelpshiftCustomDarkConfig.plist` file. For details, refer to the [Guide](#attributes-global)
6. Make sure both custom plist files are present in the same `HelpshiftCustomThemes` folder.
7. Uncomment the line `HelpshiftPostProcess.AddHelpshiftCustomThemes(project);` in `HelpshiftPostProcess.cs` to ensure the theming files are copied in the final Xcode project.
8. Call `HelpshiftSdk.setThemes(lightThemeFileName, darkThemeFileName)` API anytime after the SDK is initialized

Example Code

```csharp
HelpshiftSdk support = HelpshiftSdk.getInstance();
support.setThemes("HelpshiftCustomLightConfig", "HelpshiftCustomDarkConfig");
```
...
5. The [enableAutomaticThemeSwitching](/unity/sdk-configuration-ios#enableAutomaticThemeSwitching) config flag is ignored when this API is used and SDK will use the provided themes for Light and Dark Appearances respectively.
...
`HelpshiftSdk.setTheme(themeFileName)`

Steps to follow:

1. Navigate to the `Assets/Helpshift/Plugins/iOS/HelpshiftCustomThemes` folder.
2. Make a copy of `HelpshiftConfig.plist` and rename it to your preferred name, for eg `HelpshiftCustomConfig.plist`
...
4. Make sure the custom plist file is present in `HelpshiftCustomThemes` folder.
5. Uncomment the line `HelpshiftPostProcess.AddHelpshiftCustomThemes(project);` in `HelpshiftPostProcess.cs` to ensure the theming file is copied in the final Xcode project.
6. Call `HelpshiftSdk.setTheme(themeFileName)` API anytime after the SDK is initialized.

Example Code

```csharp
HelpshiftSdk support = HelpshiftSdk.getInstance();
support.setTheme("HelpshiftCustomConfig");
```
...
3. The [enableAutomaticThemeSwitching](/unity/sdk-configuration-ios#enableAutomaticThemeSwitching) config flag is ignored when this API is used and SDK will use the provided theme for both Light and Dark Appearances.
...
Helpshift SDK bundles themes for light and dark mode out of the box but leaves the decision to enable dark mode switching to the application developers. You can enable automatic switching of Helpshift SDK theme using [enableAutomaticThemeSwitching](/unity/sdk-configuration-ios#enableAutomaticThemeSwitching) flag.
...
1. Navigate to the `Assets/Helpshift/Plugins/iOS/HelpshiftCustomThemes` folder.
2. Change the values in `HelpshiftCustomConfig.plist` and `HelpshiftCustomConfigDark.plist`.
3. Uncomment the line `HelpshiftPostProcess.AddHelpshiftCustomThemes(project);` in `HelpshiftPostProcess.cs` to ensure the theming files are copied in the final Xcode project.
...
1. Navigate to the `Assets/Helpshift/Plugins/iOS/HelpshiftCustomThemes` folder.
2. Change the values in `HelpshiftCustomConfig.plist`.
3. Uncomment the line `HelpshiftPostProcess.AddHelpshiftCustomThemes(project);` in `HelpshiftPostProcess.cs` to ensure the theming files are copied in the final Xcode project.
...
|                                        |                                                                                                                                                      |
| -------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| Font name                              | Global font face - applies to texts for sections list, FAQ list, FAQ answer, search results and notification banner sub-title                        |
