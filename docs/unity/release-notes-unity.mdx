...
- Fixed a crash that would occur when user tapped on search results while simultaneously searching for FAQs in the search bar. Stack trace -
       ...
       `Terminating app due to uncaught exception 'NSRangeException', reason: '***-[__NSArrayM objectAtIndexedSubscript:]: index 0 beyond bounds for empty array'`**Android**
...
**iOS**- Fixed a crash that would occur when custom font was set in SDK's theming plist files.** Upgrade guide **To upgrade to 5.4.X please refer to the guide [here](/unity/upgrade-54x-ios/)**Android**
...
- Added a configuration option, hs\_\_issue_description_min_chars for setting the minimum number of characters required before you can send a message in conversational experience. For more details, [refer here](/unity/sdk-configuration-android#minimum-issue-desc-length).
...
- Fixed a bug where [enableTypingIndicator](/unity/sdk-configuration-android#enableTypingIndicator) config was not working when starting Conversation screen with _HelpshiftSdk.showConversation(configDictionary)_ api.
...
**Common Updates **- Applications are now required to build with Unity 5.3 or above. If you have a project with Unity version prior to 5.3, you could use the build mentioned [here](https://support.helpshift.com/kb/article/can-i-install-an-older-version-of-the-helpshift-sdk#unity).** iOS **
...
**Common Updates **- Fixed a bug where providing no config for [supportFileFormats](/unity/delegates-android#display-attachment) in install would cause a crash when attachment with unknown file format is opened.** Android **
