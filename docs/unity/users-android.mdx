...
Applicable to Unity SDK above 4.x and above.
...
- If you are setting [`enableFullPrivacy`](/unity/sdk-configuration-android#enableFullPrivacy) to be "yes", you shouldn't be using email as the only identifier in the login API for that user. This will result in the creation of an anonymous user.
...
- If you are using the [`leaveBreadCrumb`](/unity/tracking-android#app-usage) API to pass user login related crumbs, please make sure to call the [`clearBreadCrumbs`](/unity/tracking-android#app-usage) API when you change the login.
...
When Identity Verification is turned 'On' and a login request is made with no 'user authentication token' or with an invalid 'user authentication token' for Unity SDK v4.0.0 & above, identity verification will fail. If identity verification fails, the following will hold true:
...
End-users on Unity SDK 3.0.x or lower are not impacted, even if 'Identity Verification' is enabled on the Dashboard.
