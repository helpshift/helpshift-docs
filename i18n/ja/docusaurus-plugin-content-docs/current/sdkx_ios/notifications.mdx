...
プッシュ通知を受信すると、アプリケーションのデリゲート`didReceiveRemoteNotification:`が呼び出されます。さらに、

1. アプリケーションがフォアグラウンドになく、ユーザーがプッシュ通知をタップした場合、このメソッドが再度呼び出されます。
2. ユーザーによりアプリが強制終了させられた後にプッシュ通知がタップされた場合、アプリケーションの`didFinishLaunchingWithOptions:`デリゲートが呼び出されます。
...
`didReceiveRemoteNotification:`デリゲートの場合:
