...
- Unity 5.5.6以上。
- サポートされているAndroid OSのバージョン: 21以上
...
|                                                                      |                                                              |
| -------------------------------------------------------------------- | ------------------------------------------------------------ |
| helpshiftX-plugin-unity-[% sdkxunity.package_version %].unitypackage | Helpshift SDKのUnityパッケージ                               |
...
インストール呼び出しは、必ず`Awake()`メソッド内配置してください。これを他の場所に配置すると、実行時の予期せぬ問題が引き起こされてしまう可能性があります。
...
バージョン21未満のAndroid SDKで`install()`を呼び出しても動作しません。すべてのAPIが操作不可能となります。
