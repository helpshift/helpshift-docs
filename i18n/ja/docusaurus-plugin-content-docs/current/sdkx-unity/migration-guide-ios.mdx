...
- [ガイド付きの問題提出](/unity/support-tools-ios#dynamic-forms)
- [タグによるFAQのフィルタリング](/unity/support-tools-ios#faq-filtering)
- [問題に関する記述の長さの最小値設定](/unity/sdk-configuration-ios#issue-description-minimum-length)
- [SDKのUI文字列のカスタマイズ](/unity/string-customization-ios/)
- [ライトテーマとダークテーマの切り替え](/unity/design-ios#skinning)
...
- プロジェクトにHelpshift SDK Xを追加するには、[こちら](/sdkx-unity/getting-started-ios#adding-to-project)の手順にしたがってください。
...
XcodeからObjective-Cを利用してSDKを初期化する場合は、[こちら](/sdkx-unity/getting-started-ios#objc-install)の手順をご参照ください。
詳細については、SDK X Unityプラグインの[はじめに](/sdkx-unity/getting-started-ios/)ガイドをご参照ください。
...
その他の非推奨となった構成はすべて削除されており、もうサポートされていません。インストール構成の詳細については、[構成](/sdkx-unity/sdk-configuration-ios/)ガイドをご参照ください。
...
Conversation APIとFAQ APIの詳細については、[Helpshift API](/sdkx-unity/support-tools-ios/)ガイドをご参照ください。
...
その他の構成はすべて非推奨となっており、もうサポートされていません。API構成の詳細については、[構成](/sdkx-unity/sdk-configuration-ios/)ガイドをご参照ください。
...
SDK X Unityプラグインのユーザー管理については、[ユーザー](/sdkx-unity/users-ios/)ガイドをご参照ください。
...
SDK XのNotification APIの詳細については、[通知](/sdkx-unity/notifications-ios/)ガイドをご参照ください。SDK Xのデリゲートの詳細については、[デリゲート](/sdkx-unity/delegates-ios/)ガイドをご参照ください。
...
SDK Xのインターナショナリゼーションの詳細やサポートされている言語のリストについては、[インターナショナリゼーション](/sdkx-unity/i18n-ios/)ガイドをご参照ください。
...
レガシーSDKはメタデータAPIを活用して会話にメタデータを付与します（[こちら](/unity/tracking-ios#metadata)をご参照ください）。メタデータを付与するためのレガシーSDKコードを、同等のSDK Xコードに置き換えます。
...
詳細については、[トラッキング](/sdkx-unity/tracking-ios/)ガイドをご参照ください。
...
レガシーSDK Unityプラグインでは、SDKの各イベントは個別のメソッドを持っていました。SDK X Unityプラグインでは、すべてのイベントはデリゲートの`HandleHelpshiftEvent(eventName,eventData)`メソッドで受信されます。イベントの一覧については、[こちらのガイド](/sdkx-unity/delegates-ios#helpshift-events)をご参照ください。
...
- [手動のアプリ評価リクエスト](/unity/reviews-and-feedback-ios/#app-rating-api)
- [自動のレビューリクエスト](/unity/reviews-and-feedback-ios/#configuring-automatic-review-requests)

### ディープリンク

ディープリンクは、レガシーSDKと同様にSDK Xでもサポートされています。詳細については、[ディープリンク](/sdkx-unity/deep-linking-ios/)をご参照ください。
