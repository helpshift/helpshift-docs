...
カスタムURLスキームを使用して、HelpshiftのFAQからアプリ
内の特定のリンクへとユーザーを誘導します。これらのスキームを使用することで、より
シームレスな体験をユーザーに提供できるようになります。たとえば、あなたのAndroidアプリ内に
...
ディープリンクとは、ユーザーがカスタム
URLをクリックした際にそのURLが何を行うかを開発者が
自由に決定できるカスタムリンクURLのことを指します。
