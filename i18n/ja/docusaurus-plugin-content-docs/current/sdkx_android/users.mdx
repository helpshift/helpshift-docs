...
複数のログインユーザー/匿名ユーザーが同じデバイスを使用してサポート会話中に情報のやり取りを行う（理想としてはログインをまたいだ共有を許可したくないない）ユースケースの場合には、`Helpshift.clearAnonymousUserOnLogin(boolean clearAnonymousUser)` APIを使用する必要があります。
