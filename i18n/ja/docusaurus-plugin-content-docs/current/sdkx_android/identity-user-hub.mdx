...
SDKに含まれているすべてのパブリックAPIは、[Helpshift installWithPlatformId API](/sdkx_android/getting-started#start-using)を介してSDKを初期化した後に呼び出す必要があります
...
これらのイベントの詳細については、[デリゲート](/sdkx_android/delegates/#helpshifteventusersessionexpired)ページをご参照ください。
...
このAPIは、既存のHelpshiftデリゲートシステムを活用してエラーが発生した際にアプリに通知を行います。このAPIでトリガーできるエラーイベントの詳細については、[デリゲート](/sdkx_android/delegates.mdx/#user-identity-system-events)ページを参照してください。
...
これらの失敗の理由の定数は、`UserLoginFailureReason`ファイルで定義されています。エラーの定数は、`InvalidDataErrorReason`ファイルで定義されています。
