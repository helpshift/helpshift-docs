...
このページでは、特定のエンドユーザーを対象とした、機能に制限のある非推奨のアウトバウンドサポート機能について説明します。

弊社では、アウトバウンドサポートの強化については**プロアクティブエンゲージメント**の使用をお勧めしております。これにより、プレイヤーのモバイルデバイスにプッシュ通知やアプリ内通知を送信して連絡を取ったり、会話を開始したりするなどの様々な方法でエンゲージメントを図れるようになります。
詳細については、[プロアクティブエンゲージメント](/sdkx_android/proactive-engagement)をご参照ください。
...
アウトバウンドサポート用のリンクを生成するには、Helpshiftのダッシュボードで、「**設定**」、「**ワークフロー**」、「**アウトバウンドサポート**」の順に移動します。

![](/static/books/helpshiftx/outbound-support.png)

「**新しいリンク**」ボタンが表示されているはずです。「新しいリンク」ボタンをクリックし、**チャット、ヘルプセンター、シングルFAQ、FAQセクション**などのアクションと、Helpshift SDKにペイロードとして送信する**CIFs、タグ、最初のユーザーメッセージ**などのデータを選択します。
